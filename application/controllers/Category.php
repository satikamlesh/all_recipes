...
class Category extends CIF_Controller {
    ...
    public function index($permalink = null, $offset = 0) {
        ...
        $data['categories_side'] = $this->db
                        ...
                        ->select('categories.*,icons.icon')->where('status',1)->order_by('title')->get('categories')->result();

        //Get cagtegories data in search
        $categories[''] = 'Choose All Categories';
        foreach ($this->db->where('status',1)->get('categories')->result() as $category) {
